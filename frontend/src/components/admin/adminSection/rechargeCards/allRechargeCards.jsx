import { faCircleCheck, faCircleXmark, faDownload, faTrash } from "@fortawesome/free-solid-svg-icons";
...
function AllRechargeCards() {
    ...
    const exportToExcel = () => {
        // Create an array of selected card codes
    ...
    useEffect(() => {
        if (cardTitleId) {
            setLoading(true);
            getMethode(`${getCardsRoute}?cardTitleId=${cardTitleId}&page=${page}&limit=${limit}`).then((response) => {
        ...
        <div>
                                ...
                                cards && cards.map((card) => {
                                    return (
                                        <tr>
                                            <th>
                                                <label>
                                                    <input type="checkbox" className="checkbox" checked={listCardsSelected.includes(card._id)}
                                                        onChange={() => handleSelectItem(card._id, setListCardsSelected)} />
                                                </label>
                                            </th>
                                            <td>{card.code}</td>
                                            <td>{card.credit}$</td>
                                            <td>{card.isUsed ? <FontAwesomeIcon icon={faCircleCheck} className="text-success" /> : <FontAwesomeIcon icon={faCircleXmark} className="text-error" />}</td>
                                        </tr>
                                    )
                                })
                            }
            ...
            <div className='flex justify-between mt-[1rem]'>
                <button className='btn btn-error text-white' onClick={() => {
                    if (listCardsSelected.length > 0) document.getElementById('deleteCards').showModal()
                }}><FontAwesomeIcon icon={faTrash} /></button>
                <button className="btn btn-primary mr-2" onClick={exportToExcel}>
                    <FontAwesomeIcon icon={faDownload} />
                </button>
                <div className='w-full'>
                    <RowsPerPage page={page} setPage={setPage} limit={limit} setLimit={setLimit} totalPages={totalPages} setTotalPages={setTotalPages} totalItem={totalCards} />
                </div>
            </div>
